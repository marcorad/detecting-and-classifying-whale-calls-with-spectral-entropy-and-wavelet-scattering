...
def _create_filterbank(N: int, d: int, Q: float, startfreq: float = None, input_ds_factors: List[int] = None):
    ...
    fb, ds = _filterbank_1d(N, d, Q, startfreq, False, input_ds_factors)
    return fb, ds

def scattering_filterbanks(N, d, Q: List[float], startfreq: float = None, allow_ds = True):
    ...
    for j, q in enumerate(Q):
        fb, ds = _create_filterbank(N, d, q, startfreq if j == 0 else None, ds)
        if not allow_ds: ds = [1] 
        FB.append(fb)
    return FB

def get_wavelet_filter(fb: List, level: int, input_ds: int, lambda_: float): 
    ...
    """
    if lambda_ == 0: return fb[level][input_ds]['phi']
    return fb[level][input_ds]['psi'][lambda_]

def get_output_downsample_factor(fb: List, level: int, input_ds: int, lambda_: float) -> int:
    ...
    """
    return fb[level][input_ds]['ds'][lambda_]
...
def _for_each_filter(fb, func):
    ...
    for level in range(Nlevels):
        for ids in fb[level].keys():
            #phi
            phi = get_wavelet_filter(fb, level, ids, 0)                
            fb[level][ids]['phi'] = func(phi)
            #psi
            if fb[level][ids]['psi'] != None: #there may be some LPFs stored which do not require psi
                for l in fb[level][ids]['psi'].keys():
                    psi = fb[level][ids]['psi'][l]
                    fb[level][ids]['psi'][l] = func(psi)
...
def get_Lambda_set(fb: List, level: int, input_ds):
    ...
    """
    if fb[level][input_ds]['psi'] == None: return []
    Lambda = list(fb[level][input_ds]['psi'].keys())# list of all lambdas in each dimension
    return Lambda
